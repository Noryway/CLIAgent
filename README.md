...
```text
CLIAgent 已启动，输入 help 查看命令，exit 退出。
you> 我叫小明
assistant> 你好小明！...
you> /context
...
```text
Main
  ├── 无参数 → runRepl()：ReplCommandParser → exit/clear/help/context/对话
  ├── 有参数 → runOnce()：单次 agent.run()
  ├── EnvConfig
  ├── ToolRegistry（5 内置工具 + PathGuard/CommandGuard）
